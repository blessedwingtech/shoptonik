...
router = APIRouter()  # PAS de prefix ici, géré par seller/__init__.py
...
async def request_seller_status(
    ...
    if current_user.is_seller:
        raise HTTPException(status_code=400, detail="Vous êtes déjà vendeur")
...
async def get_seller_request_status(
    ...
    if current_user.is_seller:
        return {"status": "approved", "approved_at": current_user.seller_approved_at, "is_seller": True}
    if current_user.seller_requested_at:
        return {"status": "pending", "requested_at": current_user.seller_requested_at, "is_seller": False}
    return {"status": "none", "is_seller": False}
...
async def get_pending_seller_requests(
    ...
    for user in pending_users:
        result.append({
            ...
            "company_name": user.seller_company_name,
...
async def approve_seller_request(
    ...
    return {"message": f"La demande de {user.email} a été approuvée",
            "user": {"id": user.id, "email": user.email, "username": user.username, "is_seller": True}}
